...
When multiple saved values need selected-option hydration, the selected preload endpoint owns the response order that Tom Select restores. `rfk_find_with` follows incoming selected ID order by default when `order:` is omitted; use `order:` for relation-owned response order, and combine it with `preserve_order: true` only when the final payload must return to incoming ID order. See [`controller_helpers.md#rfk_find_with`](controller_helpers.md#rfk_find_with) for accepted `id_param:` / `ids_param:` inputs and the endpoint-side source of truth.
