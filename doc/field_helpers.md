...
| A native password, checkbox, radio, file, or range control with focused ownership boundaries, especially credential-adjacent password inputs | Use `rfk_password_field`. | Keeps password visibility toggles, strength meters, credential policy, authentication workflow, and credential storage outside Rails Fields Kit. Use [`password_field.md`](password_field.md). |
