...
function extractNodeSmokeScriptReferences(command) {
  return command
    .split(/\s*&&\s*/)
    .map((segment) => segment.trim())
    .map((segment) => segment.match(/^node(?:\s+--[^\s]+)*\s+(?<script>(?:\.\/)?scripts\/check_[^\s"']+\.mjs)(?:\s|$)/)?.groups?.script)
    .filter(Boolean)
    .map(normalizeScriptPath)
}

function evaluateInventory({ actualScripts, checkJsCommand, excludedScripts = intentionalStandaloneCheckScripts }) {
  const actualSet = new Set(actualScripts.map(normalizeScriptPath))
  const referencedScripts = extractNodeSmokeScriptReferences(checkJsCommand)
  ...
  assert.deepEqual(
    missingScripts,
    [],
    `package.json check:js must include CI-owned smoke scripts: ${missingScripts.join(", ")}`
  )
  assert.deepEqual(
    staleReferences,
    [],
    `package.json check:js references missing smoke scripts: ${staleReferences.join(", ")}`
  )
  assert.deepEqual(
    staleExclusions,
    [],
...
async function runInventoryCheck() {
  ...
  const checkJsCommand = packageJson.scripts?.["check:js"]

  assert.equal(typeof checkJsCommand, "string", "package.json must define scripts.check:js")

  const actualScripts = await smokeScriptsUnder(repoRoot)
  const { expectedScripts } = evaluateInventory({ actualScripts, checkJsCommand })
...
function runSelfTests() {
  ...
  evaluateInventory({
    actualScripts,
    checkJsCommand: "node --check app/javascript/example.js && node scripts/check_alpha.mjs && node ./scripts/check_beta.mjs",
  ...
  assertThrowsWithMessage(
    () => evaluateInventory({
      actualScripts,
      checkJsCommand: "node scripts/check_alpha.mjs",
      excludedScripts: new Set(["scripts/check_standalone.mjs"])
    }),
    /check_beta\.mjs/
  )
  ...
  assertThrowsWithMessage(
    () => evaluateInventory({
      actualScripts,
      checkJsCommand: "node scripts/check_alpha.mjs && node scripts/check_beta.mjs && node scripts/check_missing.mjs",
      excludedScripts: new Set(["scripts/check_standalone.mjs"])
    }),
    /check_missing\.mjs/
  )
  ...
  assertThrowsWithMessage(
    () => evaluateInventory({
      actualScripts,
      checkJsCommand: "node scripts/check_alpha.mjs && node scripts/check_beta.mjs && node scripts/check_standalone.mjs",
      excludedScripts: new Set(["scripts/check_standalone.mjs"])
    }),
