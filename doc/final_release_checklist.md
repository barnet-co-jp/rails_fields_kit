...
- [ ] Confirm one representative rendered field can be inspected through `import { readRenderedOptionPayloadMapping } from "rails_fields_kit"`, including the documented `search_field:` array shape and the `null` no-config lanes for `optionDescriptionField` / `optionBadgeField`, without treating request execution or visible option rendering as gem responsibility.
