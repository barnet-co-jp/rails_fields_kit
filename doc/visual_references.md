...
| [`table_metadata_visual_reference.html`](table_metadata_visual_reference.html) | Table metadata filter and cell editor lanes, including shared metadata source and responsibility boundaries | Can reviewers scan metadata-driven filter/editor examples and the host-app-owned shared source pattern without treating table persistence, query execution, or future registry APIs as Rails Fields Kit responsibilities? |
...
- For table integration boundary checks, use the index family picker or task picker to choose between metadata-driven filters/editors, shared metadata source pattern, and saved-search token suggestion states.
