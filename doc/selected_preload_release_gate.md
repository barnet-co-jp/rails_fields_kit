...
- Confirm a failure response still dispatches `rails-fields-kit--tom-select:selected-load-error` and leaves any visible fallback UI or `error_surface:` handling to the host app.
...
- `doc/controller_helpers.md` for `rfk_find_with` accepted params.
- `app/javascript/rails_fields_kit/tom_select_controller.js` for the selected preload request encoding.
- `doc/sample_app_checklist.md` and `doc/final_release_checklist.md` for the broader release checklist entries.

The release is ready only when JavaScript behavior, controller helper docs, and release checklist expectations all describe the same comma-separated multiple-ID contract.
