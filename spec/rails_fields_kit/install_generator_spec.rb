...
RSpec.describe RailsFieldsKit::Generators::InstallGenerator do
  ...
  it "uses the packaged templates directory as its source root" do
    source_paths = described_class.source_paths.map(&:to_s)

    expect(source_paths).to include(a_string_ending_with("lib/generators/rails_fields_kit/templates"))
  end

  it "ships the initializer and setup note templates" do
    source_root = described_class.source_paths.first
