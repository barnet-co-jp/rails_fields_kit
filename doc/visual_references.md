...
For `allow_clear` / clear-button review, use [`tom_select_plugin_clearable_review.html`](tom_select_plugin_clearable_review.html) as a map-only companion lane after the core Tom Select reference. It compares a single-select whole-field clear affordance with multi-item remove buttons while keeping plugin assets, styling, event payloads, selection mutation, and Tom Select lifecycle behavior outside Rails Fields Kit.

For package-root contract reader review, use [`public_api.md#javascript-exports`](public_api.md#javascript-exports) as the source of truth for the current landed helper list and return-shape boundary before choosing a visual lane. Route Tom Select rendered-state questions through the core Tom Select reference or text override copy lane, native wrapper and accessibility questions through the native helper states or native accessibility contract lane, and error-surface or request-reader questions through the request-failure feedback lane. The visual reference index should name these reviewer tasks instead of copying the JavaScript export inventory or naming proposal-only helpers as current API.
