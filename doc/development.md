...
The rendered option payload mapping smoke keeps rendered option mapping data readable from the package root: explicit value, label, search, description, and badge field names are visible, default mapping falls back to `value` / `text`, blank optional fields normalize to `null`, and unrelated elements return `null`.
