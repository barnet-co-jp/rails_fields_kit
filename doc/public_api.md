...
`readRenderedOptionPayloadMapping(element)` returns `{ valueField, labelField, searchFields, optionDescriptionField, optionBadgeField }` for a rendered Rails Fields Kit Tom Select field and returns `null` otherwise. It only reads the rendered public contract. Request execution, option rendering HTML, and visible feedback remain host-app responsibility.
