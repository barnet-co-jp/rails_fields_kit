...
RSpec.describe "Tom Select error events" do
  ...
  it "includes response metadata in the shared error detail helper" do
  ...
  it "documents the normalized failure event detail shape" do
    ...
    expect(events_doc).to include("Detail: `{ operation, query, error, response, payload, status }`")
    expect(events_doc).to include("Detail: `{ operation, values, error, response, payload, status }`")
    expect(events_doc).to include("Detail: `{ operation, input, error, response, payload, status }`")
  end
end
