...
- Use native wrapper helpers when a native browser input is enough and you only want Rails Fields Kit's shared labels, hints, validation errors, affixes, and accessibility wiring. Keep the exact helper inventory in [`doc/public_api.md`](doc/public_api.md#formbuilder-helpers), then choose the focused guide by job:
