...
| A native browser input that only needs Rails Fields Kit's shared wrapper behavior | Native wrapper family, starting with `rfk_text_field`, `rfk_text_area`, and `rfk_search_field`; use focused native helpers such as `rfk_password_field` or `rfk_range_field` when their boundaries match | Stays in the ordinary HTML input flow while reusing wrapper, hint, error, affix, and accessibility conventions. Use [`doc/public_api.md`](public_api.md) for the current helper inventory, [`password_field.md`](password_field.md) for password-specific non-goals, and [`range_field.md`](range_field.md) for browser-native slider boundaries. |
