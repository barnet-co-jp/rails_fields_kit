...
The package root also exposes a helper for reading the documented text override contract from a rendered field:

```js
import { readRenderedTextOverrides } from "rails_fields_kit"
```

`readRenderedTextOverrides(element)` returns `{ noResultsText, loadingText, createText }` for a rendered Rails Fields Kit Tom Select field and returns `null` otherwise. It reads the final rendered copy after field-level overrides, initializer defaults, and bundled locale-aware fallback have already been resolved, while keeping visible copy ownership in the host app.
...
For host-app code that wants to inspect a rendered field's final text override contract without re-encoding raw data attribute names or render-time fallback precedence, use `readRenderedTextOverrides(element)` from the package root.
