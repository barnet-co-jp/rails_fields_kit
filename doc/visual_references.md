...
For `rfk_file_field` review, use the native helper reference's file input boundary lane. It keeps label, hint, error, required marker, `accept:`, `multiple:`, and `direct_upload:` pass-through visible while leaving multipart setup, Active Storage JavaScript, previews, progress, storage, scanning, and validation policy with the host app.
...
| [`native_field_visual_reference.html`](native_field_visual_reference.html) | Native helper wrapper, label, hint, prefix, suffix, required marker, readonly, disabled, validation, multiline textarea, file input boundary, accessibility contract, browser semantics, metadata and constraint attributes, and narrow viewport stress states | Do native helper states remain legible, including long-label, affix-heavy, multiline textarea, file upload boundary, accessibility-contract, search/email/URL semantics, and metadata/constraint boundary lanes, while staying separate from Tom Select-backed behavior and giving release reviewers a concrete lane to record? |
...
- For native helper wrapper review, use [`native_field_visual_reference.html`](native_field_visual_reference.html) to check label, hint, affix, required marker, validation, multiline textarea, file input boundary, noneditable, accessibility, browser semantics, generated-id, metadata and constraint attributes, and narrow viewport stress lanes without treating the static artifact as production CSS or runtime behavior.
...
- For native file field review, use [`native_field_visual_reference.html`](native_field_visual_reference.html)'s file input boundary lane to confirm wrapper, hint, validation, `accept:`, `multiple:`, and `direct_upload:` remain readable while upload progress, previews, storage, scanning, multipart setup, and validation policy stay host-app responsibilities.
