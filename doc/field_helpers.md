...
| A native password, checkbox, radio, file, or range control with focused ownership boundaries | Use `rfk_password_field`, `rfk_check_box`, `rfk_radio_button`, `rfk_file_field`, or `rfk_range_field`. | Keeps password policy, Rails checkbox submission, Rails radio value / checked-state behavior, upload handling, and range preview behavior in their focused docs instead of making this chooser a full guide. Use [`password_field.md`](password_field.md), [`check_box.md`](check_box.md), [`radio_button.md`](radio_button.md), [`file_field.md`](file_field.md), or [`range_field.md`](range_field.md). |
