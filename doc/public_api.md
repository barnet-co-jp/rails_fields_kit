...
See [`file_field.md`](file_field.md) for the current `rfk_file_field` thin wrapper boundary and upload workflow non-goals.
...
Collection checkbox / radio group helpers are also not current public APIs. Host apps should keep using ordinary Rails collection helpers or host-app markup for group semantics until a future helper is merged and listed here. Future proposal names, open PR helper names, and collection group helpers must not be read as current collection group API.
