...
For plugin / clearable review, use [`tom_select_plugin_clearable_review.html`](tom_select_plugin_clearable_review.html) as a companion lane to the Tom Select core reference. It compares `allow_clear: true` whole-field clear affordance against tags/token-style per-item remove buttons, while keeping plugin assets, lifecycle behavior, clear event payloads, and production styling in host-app or Tom Select ownership.
...
| [`tom_select_plugin_clearable_review.html`](tom_select_plugin_clearable_review.html) | Companion Tom Select plugin / clearable review lane for whole-field clear, per-item remove, and host-owned plugin boundary states | Can reviewers distinguish `allow_clear: true` clear affordance from tags/token-style remove buttons without treating plugin assets, lifecycle, clear events, or production styling as Rails Fields Kit-owned behavior? |
...
- For plugin / clearable review, open [`tom_select_plugin_clearable_review.html`](tom_select_plugin_clearable_review.html) and compare whole-field clear against per-item remove controls at desktop and 390px-equivalent widths. Treat it as a companion artifact to the Tom Select core reference, not as an index-level JavaScript export inventory.
