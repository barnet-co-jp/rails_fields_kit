...
- [ ] Confirm the package-root `readRenderedSelectedPreloadConfig(...)` helper resolves in the sample app, returns the documented config for that representative selected preload field, and stays `null` for a comparable field without `selected_url:`.
