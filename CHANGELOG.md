...
- `tomSelectRequestParamsContract(element)` from the package root for host-app scripts that need to read rendered fixed request params, including `queryParams`, `selectedQueryParams`, and `createParams`, without executing requests, authorizing endpoints, or owning visible feedback.
