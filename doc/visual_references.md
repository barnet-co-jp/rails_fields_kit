...
| [`tom_select_request_failure_visual_reference.html`](tom_select_request_failure_visual_reference.html) | Focused `error_surface: true` request-failure states for Tom Select-backed helpers | Can reviewers inspect hidden, revealed, restore-failure, create-failure, and custom-wrapper feedback without treating retry UI or request lifecycle behavior as built in? |
...
- For Tom Select request-failure feedback, use [`tom_select_request_failure_visual_reference.html`](tom_select_request_failure_visual_reference.html) to review the `error_surface: true` slot while keeping retry copy, reveal timing, and request lifecycle behavior with the host app.
