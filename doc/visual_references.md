...
For helper choice review, use [`helper_choice_comparison.html`](helper_choice_comparison.html) as a companion artifact before choosing a detailed Tom Select lane. It compares `rfk_select`, `rfk_combobox`, `rfk_autocomplete`, `rfk_tags`, and `rfk_token_search` by rendered state, submitted value shape, endpoint responsibility, and host-app ownership without making proposal-only helper names or runtime behavior current API.
...
| [`helper_choice_comparison.html`](helper_choice_comparison.html) | Companion comparison for choosing between `rfk_select`, `rfk_combobox`, `rfk_autocomplete`, `rfk_tags`, and `rfk_token_search` before opening a detailed helper lane | Can reviewers quickly compare rendered state, submitted value shape, endpoint responsibility, and host-app ownership without treating the companion artifact as helper implementation or public API documentation? |
...
- For helper choice review, open [`helper_choice_comparison.html`](helper_choice_comparison.html) before choosing a detailed Tom Select or token-search reference. Use it to compare submitted value shape, endpoint requirements, and ownership boundaries; use helper topic docs for exact options and behavior.
