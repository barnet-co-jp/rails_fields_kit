...
- [ ] Confirm representative package-root export evidence before release. Use this matrix as release-critical sample coverage, not as the full helper inventory; first scan `doc/public_api.md#javascript-exports` and `doc/package_root_helper_release_evidence.md` for the current helper names, return shapes, and evidence lane selection.
  ...
  - [ ] One request/config reader lane resolves from `rails_fields_kit` and is checked through the documented return shape, such as `tomSelectRequestContract(element)`, `readRenderedSelectedPreloadConfig(element)`, or `readRenderedTomSelectInteractionConfig(element)`, without moving request execution, endpoint authorization, selected-preload fallback UI, selector validation, modal / portal layout, or production CSS into the package.
  - [ ] One rendered-state reader lane resolves from `rails_fields_kit` and is checked through the documented return shape, such as `tomSelectSelectionContract(element)`, `tomSelectFieldKindContract(element)`, `readRenderedErrorSurface(element)`, `readRenderedOptionPayloadMapping(element)`, or `readRenderedTableFilterMetadata(element)`, without mutating selections, redefining helper taxonomy, creating feedback UI, executing endpoints, parsing token strings, running Ransack, or deciding table search behavior.
...
- [ ] Confirm any release-scoped package-root helper not covered by the representative rows above has an evidence location in `doc/sample_app_results.md` or the release PR comment, with helper names and return-shape details still sourced from `doc/public_api.md#javascript-exports`.
