...
For a copyable host-app example that reads the rendered metadata surface and turns submitted token text into `params[:q]`, see [`ransack_suggestions.md`](ransack_suggestions.md#copyable-read-path-for-table-rendered-metadata) and [`ransack_suggestions.md`](ransack_suggestions.md#copyable-host-app-parser-example).
