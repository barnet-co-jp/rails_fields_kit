...
| `scripts/check_package_exports.mjs` via `npm run check:js` | Importability and callable smoke for the current exports derived from the public API table, plus targeted helper smokes when a generic callable check is not enough. | Runtime request execution, endpoint authorization, visible feedback, or sample-app release evidence. |
