...
- `readRenderedRansackFilterMetadata(...)` as a package-root helper for reading `{ adapter, paramName, fields }` from `TableFilterInput.ransack_filter` fields rendered through the table helpers.
...
- `readRenderedRansackFilterMetadata(element)` only reads metadata that was already rendered into the field. It does not take over parsing or query execution.
...
- when the package-root helper export is in the release scope, `import { readRenderedRansackFilterMetadata } from "rails_fields_kit"` resolves and a representative table-rendered field returns `{ adapter, paramName, fields }`.
...
```markdown
Rails Fields Kit 0.1.1 expands the gem beyond the first 0.1.0 release with token-oriented search helpers, metadata-only Ransack suggestion builders, table adapter metadata for rendering documented field helpers through existing host-app table definitions, and a package-root helper for reading rendered Ransack metadata from those table fields.
...
- `readRenderedRansackFilterMetadata(...)` from `rails_fields_kit`
...
Rails Fields Kit still stops at UI helpers and metadata. Host applications remain responsible for token parsing, search execution, authorization, pagination, and JavaScript package manager or importmap choices. `readRenderedRansackFilterMetadata(...)` only reads metadata that the table-rendered field already exposes.
