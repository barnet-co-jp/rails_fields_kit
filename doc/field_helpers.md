...
| A native text, textarea, or search field with shared wrapper, hint, error, affix, and accessibility behavior | Start with `rfk_text_field`, `rfk_text_area`, or `rfk_search_field`. | Stays in the ordinary HTML input flow while reusing Rails Fields Kit wrapper conventions. Use [`doc/public_api.md`](public_api.md) for the current helper inventory and [`native_contact_fields.md`](native_contact_fields.md) for contact/search ownership boundaries. |
| A native password, checkbox, file, or range control with focused ownership boundaries | Use `rfk_password_field`, `rfk_check_box`, `rfk_file_field`, or `rfk_range_field`. | Keeps password policy, Rails checkbox submission, upload handling, and range preview behavior in their focused docs instead of making this chooser a full guide. Use [`password_field.md`](password_field.md), [`check_box.md`](check_box.md), [`file_field.md`](file_field.md), or [`range_field.md`](range_field.md). |
| Native numeric, money, percent, email, URL, or phone inputs | Use `rfk_number_field`, `rfk_money_field`, `rfk_percent_field`, `rfk_email_field`, `rfk_url_field`, or `rfk_phone_field`. | Keeps formatting, rounding, normalization, validation wording, and phone policy with the host app while Rails Fields Kit owns the shared wrapper lane. Use [`native_numeric_fields.md`](native_numeric_fields.md) and [`native_contact_fields.md`](native_contact_fields.md). |
