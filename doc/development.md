...
This command checks the public package entrypoint and Tom Select controller source without installing additional npm dependencies. It also runs lightweight Node sandbox checks for package `exports` import wiring, Tom Select create-on-the-fly JSON request headers, Tom Select error-surface metadata, and Tom Select remote option label fallback, stubbing external browser dependencies so the package root and direct controller entrypoint are resolved through the same public import paths CI uses.
...
The label fallback smoke keeps the remote option renderer contract visible: explicit label fields win, missing or blank labels fall back to the configured value field, and `0` / `false` labels remain present display values.
...
- `npm run check:js` on Node 22.x for the JavaScript syntax, package exports import lane, Tom Select create request header smoke, Tom Select error surface smoke, and Tom Select label fallback smoke
