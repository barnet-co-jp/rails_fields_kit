...
`readRenderedCreateOnTheFlyConfig(element)` returns `{ createUrl, createParam, createParams }` for a field that opts into `create_url:` and returns `null` otherwise. It only reads the rendered public contract. Request execution, visible feedback, and retry UI remain host-app responsibility.
