...
- For native non-editable state evidence, use [`field_helpers.md`](field_helpers.md) as the boundary source and record separate comparable fields for `disabled:` and `readonly:`. Confirm each field still shows its current value through the shared wrapper and keeps the scoped label, hint, prefix, suffix, and accessibility wiring. Do not treat disabled and readonly as interchangeable, and do not infer authorization or business-rule ownership from rendered native state.
