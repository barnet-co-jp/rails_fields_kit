...
For a static review surface that shows this boundary without adding runtime behavior, see [`masked_input_boundary_visual_reference.html`](masked_input_boundary_visual_reference.html). Treat that artifact as visual evidence for host-owned masking hooks, not as a new helper or public API proposal.
