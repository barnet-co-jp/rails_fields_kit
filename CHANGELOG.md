...
- Package-root JavaScript named export `readRenderedRansackFilterMetadata(element)` for reading table-rendered Ransack metadata as `{ adapter, paramName, fields }` from rendered fields while leaving token parsing and query execution in the host application.
