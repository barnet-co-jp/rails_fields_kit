...
- `doc/table_date_time_color_metadata.md`: focused table metadata guide for `TableFilterInput.date_field` / `time_field` / `datetime_local_field` / `color_field` and the matching `TableCellInput` factories; picker behavior, timezone conversion, masking, query execution, table persistence, and production styling stay out of scope
